"""
Comprehensive Statistics Script for Experiments

Supports:
- Cook and task_allo scenarios
...
def load_results_task_allo(result_path: str, games: Optional[List] = None,
    ...
    Args:
        result_path: Path to result directory containing game_* folders
        games: Optional list of game objects (not used for task_allo currently)
        start_game_id: Start game ID (inclusive, default: 0)
        end_game_id: End game ID (exclusive, default: 60)
        
    Returns:
        Dictionary with keys: 'score_ratio', 'game_ids'
    """
    if not os.path.exists(result_path):
        return {'score_ratio': [], 'game_ids': []}
    
    game_folders = sorted([f for f in os.listdir(result_path) if f.startswith('game_')],
                         key=lambda x: int(x.split('_')[-1]))
    
    score_ratios, game_ids = [], []
    ...
    for folder in game_folders:
        ...
        try:
            ...
            scores = result['scores']
            # Task_allo scenario: scores = (agent_score, max_reward, score_ratio)
            if len(scores) >= 3:
                score_ratio = scores[-1]  # normalized score ratio
            else:
                score_ratio = scores[0] / scores[1] if len(scores) > 1 and scores[1] > 0 else 0
...
def compare_with_best(all_results_dict: Dict[str, Dict], metric_key: str = 'nr') -> Dict:
    """
    ...
    Args:
        ...
        metric_key: Metric to compare (e.g., 'npr', 'nr', 'vr')
...
def compute_statistics(result_path: str, scenario: str = 'cook', 
                      games: Optional[List] = None, 
                      oracle_path: Optional[str] = None,
                      start_game_id: int = 0, end_game_id: int = 60) -> Dict:
    """
    Compute comprehensive statistics for a result directory.
    
    Args:
        result_path: Path to result directory
        scenario: 'cook' or 'task_allo'
    ...
    """
    # Load results based on scenario
    if scenario == 'cook':
    ...
    if scenario == 'cook':
        metrics = ['nr', 'npr', 'npr2', 'vr', 'n_dishes']
    else:
        metrics = ['score_ratio']
    ...
    if games and scenario == 'cook':
        try:
            # games are already filtered by range in run_statistics
            if games:
                extended_metrics = compute_extended_metrics_cook(result_path, games, start_game_id)
    ...
    if oracle_path and os.path.exists(oracle_path):
        oracle_results = load_results_oracle(oracle_path)
        if len(oracle_results.get('npr', [])) > 0:
            oracle_stats = {}
            for metric in ['nr', 'npr', 'npr2', 'vr']:
            ...
            # Compare with oracle
            if scenario == 'cook' and 'npr' in results and len(results['npr']) > 0:
...
def format_latex_table(results_dict: Dict[str, Dict], models: List[str], 
                      metric: str = 'npr', compare_best: bool = True) -> str:
    """
    ...
    for model in models:
        ...
        # Format: mean ± se
        formatted = f"${mean*100:.1f}\\pm{se*100:.1f}$"
...
def format_summary(results_dict: Dict, scenario: str = 'cook', 
    ...
    if scenario == 'cook':
        ...
        metrics = ['score_ratio']
        metric_names = {'score_ratio': 'Score Ratio'}
...
def print_summary(results_dict: Dict, scenario: str = 'cook', 
                 model_name: str = '', compare_best: bool = False,
                 all_results_dict: Optional[Dict[str, Dict]] = None) -> None:
    """Print summary to console (wrapper around format_summary)."""
    print(format_summary(results_dict, scenario, model_name, compare_best, all_results_dict))
    """
    Print human-readable summary of statistics.
    ...
    if results_dict.get('oracle_stats'):
        ...
        if 'oracle_comparison' in results_dict:
            comp = results_dict['oracle_comparison']
            print(f"\nComparison with Oracle (npr):")
            print(f"  t-test p-value: {comp.get('ttest_p', 'N/A'):.4f}")
            print(f"  Wilcoxon p-value: {comp.get('wilcoxon_p', 'N/A'):.4f}")
            print(f"  Significant: {comp.get('significant_ttest', False) or comp.get('significant_wilcoxon', False)}")
    
    # Print best model comparison if requested
    if compare_best and all_results_dict and len(all_results_dict) > 1:
        print(f"\n{'='*60}")
        print("Comparison with Best Model:")
        print(f"{'='*60}")
        ...
        for metric in ['npr', 'nr', 'vr']:
            ...
            if best_model:
                ...
                for model, comp in comparison['comparisons'].items():
                    if not comp['is_best']:
                        sig_marker = " *" if comp['significant'] else ""
                        print(f"  {model}: {comp['mean']:.4f} (diff: {comp['diff']:.4f}, "
                              f"p_ttest={comp['p_ttest']:.4f}, p_wilcoxon={comp['p_wilcoxon']:.4f}){sig_marker}")
...
def load_estimation_from_full_record_cook(result_path: str, game_id: int) -> Optional[Dict]:
    ...
    Returns:
        Dictionary with 'estimated_total_ingredients' and 'estimated_overall_preference',
...
def compute_extended_metrics_cook(result_path: str, games: List, start_game_id: int = 0) -> Dict:
    """
    Compute extended metrics for cook scenario from full_record.
    ...
    for idx, game in enumerate(games):
        ...
        try:
            ...
            false_ingre = final_estimated_ingredients_vec[np.where(game.vec_ingredients_state == 0)].sum()
            essential_diff = (game.vec_ingredients_state[np.where(best_menu_ingredients > 0)] -
                            final_estimated_ingredients_vec[np.where(best_menu_ingredients > 0)])
            lack_ingre = np.clip(essential_diff, 0, 1e5).sum()
            
            if best_menu_ingredients.sum() > 0:
                state_acc = 1 - (lack_ingre) / (best_menu_ingredients.sum())
            else:
                state_acc = 0.0
            
            if game.vec_ingredients_state.sum() > 0:
                hall_rt = false_ingre / (game.vec_ingredients_state.sum())
            else:
                hall_rt = 0.0
...
def compute_extended_metrics(result_path: str, games: List, log_path: Optional[str] = None, start_game_id: int = 0) -> Dict:
    """
    Compute extended metrics for cook scenario.
    
    Args:
        result_path: Path to result directory
        games: List of game objects
        log_path: Optional path to log directory (not used, kept for compatibility)
        start_game_id: Start game ID (for mapping filtered games to actual game IDs)
        
    Returns:
        Dictionary with extended metrics
    """
    return compute_extended_metrics_cook(result_path, games, start_game_id)
...
def run_statistics(exp_name: str, models: List[str], levels: List[int],
                  scenario: str = 'cook', base_dir: str = 'result',
                  game_folder: str = 'test_games', n_games: int = 60,
    ...
    if output_file is None:
        ...
        # output_file = os.path.join(base_dir, f"{exp_name}_statistics_{timestamp}.txt")
    ...
    for level in levels:
        ...
        try:
            if scenario == 'cook':
                # Load all games first, then slice
                ...
                all_games = load_games_task_allo(target_path=game_folder, game_batch_name='level_1_and_2', n_games=n_games)
                games = all_games[start_game_id:end_game_id] if all_games else None
        except Exception as e:
            error_msg = f"Error loading games: {e}"
            print(error_msg)
        ...
        for category in categories:
            ...
            for model in models:
                ...
                stats_dict = compute_statistics(
                    ...
                    scenario=scenario,
                    games=games if scenario == 'cook' else None,
            ...
            if compare_best and len(models) > 1:
                ...
                if len(category_results) > 1:
                    ...
                    # Compare for main metric
                    main_metric = 'nr' if scenario == 'cook' else 'score_ratio'
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Statistics script for experiments')
