#!/usr/bin/env python
# -*- coding: UTF-8 -*-
"""Task allocation Level 1 baseline entry script."""
...
from delib_collab.agents.task_allocation.level1.baseline_without_tools import ProcessTaskAllocationLevel1BaselineNoTools
from delib_collab.agents.task_allocation.level1.baseline_with_tools import ProcessTaskAllocationLevel1BaselineWithTools
...
parser = argparse.ArgumentParser(description='Process Task Allocation Level 1 Baseline')
...
parser.add_argument('--exp_name', type=str, default='baseline_task_allocation', help='experiment name')
...
parser.add_argument('--no_tools', default=False, action='store_true', 
                    help='use baseline without tools (default: with tools)')
...
if __name__ == '__main__':
    ...
    print(f"="*60)
    print(f"Task Allocation Level 1 Baseline Entry")
    print(f"="*60)
    print(f"Loading games from {game_folder}, start: {start}, end: {end}...")
    print(f"Model: {model}, exp_name: {exp_name}, override: {override}")
    print(f"Baseline Type: {'No Tools' if no_tools else 'With Tools'}")
    print(f"Max Character: {max_character}")
    try:
    ...
    for i, game in enumerate(games):
        ...
        print(f"\n{'='*60}")
        print(f"Processing Game {game_idx} (BASELINE)")
        print(f"{'='*60}")
        ...
        log_folder_path = os.path.join(exp_name, 'task_allocation_level_1_baseline_no_tools' if no_tools else 'task_allocation_level_1_baseline_with_tools')
        result_folder_name = os.path.join(exp_name, 'task_allocation_level_1_baseline_no_tools' if no_tools else 'task_allocation_level_1_baseline_with_tools', f'game_{game_idx}')
        
        if no_tools:
            baseline_process = ProcessTaskAllocationLevel1BaselineNoTools(
                game, 
                LLM_model_name=model, 
                max_character=max_character,
                log_folder=log_folder_path,
                log_name='baseline_no_tools', 
                game_id=game_idx,
                record_folder_name=result_folder_name,
                game_level='level_1',
                gen_max_retries=5
            )
        else:
            baseline_process = ProcessTaskAllocationLevel1BaselineWithTools(
                game, 
                LLM_model_name=model, 
                max_character=max_character,
                log_folder=log_folder_path,
                log_name='baseline_with_tools', 
                game_id=game_idx,
                record_folder_name=result_folder_name,
                game_level='level_1',
                gen_max_retries=5
            )
        if override or not os.path.exists(baseline_process.short_result_file_path):
            print(f"\nStarting baseline experiment for Game {game_idx}...")
            try:
                final_proposal, score, game_metrics = baseline_process.process
                print(f"\nGame {game_idx} completed.")
                print(f"  Final Proposal: {final_proposal}")
                print(f"  Score: {score}")
                print(f"  Metrics: NR={game_metrics['nr']:.2f}, NAR={game_metrics['nar']:.2f}, IsValid={game_metrics['is_valid']}")
            ...
            print(f"  Result file: {baseline_process.short_result_file_path}")
            try:
                import pickle
                with open(baseline_process.short_result_file_path, 'rb') as f:
    ...
    if all_game_metrics:
        ...
        if no_tools:
            metrics_output_path = os.path.join('logs', exp_name, 'task_allocation_level_1_baseline_no_tools', 'metrics_summary.json')
        else:
            metrics_output_path = os.path.join('logs', exp_name, 'task_allocation_level_1_baseline_with_tools', 'metrics_summary.json')
