#!/usr/bin/env python
# -*- coding: UTF-8 -*-
"""Task allocation Level 1 processing class (with tools)."""
...
class ProcessTaskAllocationLevel1WithToolV1:
    """Task allocation Level 1 processing class (with tools)."""
    
    def __init__(self, game, LLM_model_name="model-name", max_conversation_rounds=10, max_character=3000,
                 log_folder=None, log_name="task_allocation_level_1_v1_record", game_id=0, 
    ...
    def _format_task_requirements(self):
        """Format task requirements into a natural language string."""
        nl_requirements = []
        for task_name, requirements in self.game.task_requirements.items():
            parts = []
            if 'agent_0' in requirements and requirements['agent_0']:
                agent_reqs = requirements['agent_0']
                non_zero_reqs = {k: v for k, v in agent_reqs.items() if v != 0}
                if non_zero_reqs:
                    req_str = ', '.join([f"{k}: {v}" for k, v in non_zero_reqs.items()])
                parts.append(f"needs ({req_str})")
            
            if parts:
                nl_requirements.append(f"- {task_name}: {', '.join(parts)}")
        
        return '\n'.join(nl_requirements)
    ...
    def single_agent_step_process(self, **kwargs):
        ...
        # ================ SOLVER & CALCULATOR ========================
        kwargs['best_allocation'], reward = tools_pool.agent_call_solver(
            self.game, kwargs['total_resources'], kwargs['overall_preferences']
        )
        kwargs['expected_reward'] = reward
        print("Best allocation from solver:", kwargs['best_allocation'])
        print("Expected reward from solver:", reward)
        available_allocations, insufficient_allocation_info = tools_pool.agent_call_task_calculator(
            self.game, kwargs['total_resources']
        )
        kwargs['available_allocations'] = available_allocations
        print(f"Available allocations from calculator: {available_allocations}")
        kwargs['insufficient_allocation_info'] = insufficient_allocation_info
        print(f"Insufficient allocation info from calculator: {insufficient_allocation_info}")
        self.record["round {}".format(kwargs["round"])][kwargs['agent_name']]["observer_agent"]["solver_calculator"] = {
            "best_allocation": kwargs['best_allocation'],
            "expected_reward": reward,
            "available_allocations": kwargs['available_allocations'],
            "insufficient_allocation_info": kwargs['insufficient_allocation_info']
        }
    ...
    def process(self):
        ...
        for round in range(1, self.max_conversation_rounds + 1):
            ...
            task_names_list = self.game.tasks  # e.g., ['Data Analysis Report', 'Frontend Development', 'Backend API Development']
            task_names_str = json.dumps(task_names_list, ensure_ascii=False)
            example_prefs = {}
            for i in range(3):
                example_prefs[f"<agent_id_{i}>"] = {task: "<efficiency_value>" for task in task_names_list}
            task_names_example_preferences = json.dumps(example_prefs, ensure_ascii=False, indent=4)
            example_alloc = {}
            for task in task_names_list:
                example_alloc[task] = "<assigned_agent>"
            task_names_example_allocation = json.dumps(example_alloc, ensure_ascii=False)
            example_partner_resources = {}
            for i in range(3):
                example_partner_resources[f"<agent_id_{i}>"] = {
                    "private_resources": {"ResourceType1": "<value>", "ResourceType2": "<value>"},
                    "public_resources": {"PublicResourceType": "<value>"}
                }
            task_names_example_partner_resources = json.dumps(example_partner_resources, ensure_ascii=False, indent=4)
            example_total_resources = {
                "agent_private_resources": {
                    f"<agent_id_{i}>": {"ResourceType1": "<value>", "ResourceType2": "<value>"}
                    for i in range(3)
                },
                "public_resources": {"PublicResourceType": "<sum_of_all_agents_observations>"}
            }
            task_names_example_total_resources = json.dumps(example_total_resources, ensure_ascii=False, indent=4)
            ...
            agent_0_current_args = {
                ...
                "task_names_example_preferences": task_names_example_preferences,
                "task_names_example_allocation": task_names_example_allocation,
                "task_names_example_partner_resources": task_names_example_partner_resources,
                "task_names_example_total_resources": task_names_example_total_resources
            }
...
if __name__ == '__main__':
    """Test entry point."""
    print("ProcessTaskAllocationLevel1WithToolV1 module loaded successfully.")
    print("Use task_allocation_level_1_entry.py to run the full pipeline.")
