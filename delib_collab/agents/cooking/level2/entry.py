...
from delib_collab.agents.cooking.level2.process_with_tools import ProcessLevel2WithToolV1
from delib_collab.agents.cooking.level2 import process_without_tools


parser = argparse.ArgumentParser(description='Process Level 2')
...
if __name__ == '__main__':
    ...
    for i, game in enumerate(games):
        ...
        if not no_tools:
            process_level_2_v0 = ProcessLevel2WithToolV1(
                game, LLM_model_name=model, max_conversation_rounds=max_round, max_character=max_character,
                log_folder=os.path.join(exp_name, 'level_2_with_tools'), log_name='v2', game_id=game_idx,
                record_folder_name=os.path.join(exp_name, 'level_2_with_tools', f'game_{game_idx}'), game_level='level_2'
            )
        else:
            process_level_2_v0 = process_without_tools.ProcessLevel2WithToolV1(
                game, LLM_model_name=model, max_conversation_rounds=max_round, max_character=max_character,
                log_folder=os.path.join(exp_name, 'level_2_no_tools'), log_name='v2', game_id=game_idx,
                record_folder_name=os.path.join(exp_name, 'level_2_no_tools', f'game_{game_idx}'), game_level='level_2'
            )
        if override or not os.path.exists(process_level_2_v0.short_result_file_path):
            final_proposal, score = process_level_2_v0.process
        else:
