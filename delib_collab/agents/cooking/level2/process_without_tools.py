...
from delib_collab.agents.cooking.level2.agent import BuiltAgentLevel2
from delib_collab.agents.cooking.level2.schemas import ObseverFormat, ThinkingFormat, DecisionProcessFormat, \
    MessageFormat
from delib_collab.prompts.cooking.level2.without_tools import OBSERVE_PROMPT, THINK_PROMPT, DECISION_PROMPT, TALK_PROMPT
...
class ProcessLevel2WithToolV1:
    def __init__(self, game, LLM_model_name="model-name", max_conversation_rounds=10, max_character=3000,
                 ...
                 game_level='level_2', gen_max_retries=3):
        self.game = game
        self.LLM_model_name = LLM_model_name
        self.builtAgents = BuiltAgentLevel2(LLM_model_name=self.LLM_model_name, agent_type='react')
    ...
    def single_agent_step_process(self, **kwargs):
        ...
        while retries < self.max_retries:
            try:
                ...
                retries += 1
                self.logger.error(
                    f"Error occurred while executing observer_agent (attempt {retries}/{self.max_retries}): {e}")
    ...
    def process(self):
        ...
        for round in range(1, self.max_conversation_rounds + 1):
            ...
            Alice_guest_info_1 = self.game.agent_0_value_obs_l2[0]
            Alice_guest_info_2 = self.game.agent_0_value_obs_l2[1]
            Bob_obs = self.game.agent_1_obs
            Bob_guest_info_1 = self.game.agent_1_value_obs_l2[0]
            Bob_guest_info_2 = self.game.agent_1_value_obs_l2[1]
            self.logger.info("=======================round {}=================================".format(round))
            self.record["round {}".format(round)] = {}
            self.record["round {}".format(round)]['Alice_obs'] = Alice_obs
            self.record["round {}".format(round)]['Alice_guest_info_1'] = Alice_guest_info_1
            self.record["round {}".format(round)]['Alice_guest_info_2'] = Alice_guest_info_2
            self.record["round {}".format(round)]['Bob_obs'] = Bob_obs
            self.record["round {}".format(round)]['Bob_guest_info_1'] = Bob_guest_info_1
            self.record["round {}".format(round)]['Bob_guest_info_2'] = Bob_guest_info_2
            ...
            agent_Alice_current_args = {
                ...
                "person_1_info": Alice_guest_info_1,
                "person_2_info": Alice_guest_info_2,
            ...
            agent_Bob_current_args = {
                ...
                "max_character": self.max_character,
                "current_obs": Bob_obs,
                "person_1_info": Bob_guest_info_1,
                "person_2_info": Bob_guest_info_2,
                "chat_history": chat_history,
                "guest_name": "Guest 2",
