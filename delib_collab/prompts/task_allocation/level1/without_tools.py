#!/usr/bin/env python
# -*- coding: UTF-8 -*-
"""Task allocation Level 1 prompt templates (without tools version)."""
...
**CRITICAL - Resource Constraint Verification:**
You MUST explicitly verify whether the proposal satisfies resource constraints:

1. **Calculate Total Resource Demand**: For the partner's proposal, sum up resource requirements:
   - For each agent, sum the resource needs of tasks assigned to them (check private resources)
   - Sum public resource needs across ALL tasks in the proposal

2. **Compare with Available Resources**:
   - Private: For each agent, verify that their assigned tasks' demands ≤ their private resource capacity
   - Public: **CRITICAL** - Verify that total public resource demand ≤ **TOTAL pooled public resources** (sum of all agents' observations), NOT just your own observed portion. Public resources are shared, so you must use the combined total from all agents.

3. **Example Verification**:
   If proposal assigns tasks T1, T2, T3:
   - T1 to agent_0 (needs Time=2, Energy=3, Budget=100)
   - T2 to agent_1 (needs Time=3, Energy=2, Budget=50)
   - T3 to agent_2 (needs Time=1, Energy=4)
   
   Check:
   - agent_0: Does agent_0 have Time≥2 AND Energy≥3? 
   - agent_1: Does agent_1 have Time≥3 AND Energy≥2?
   - agent_2: Does agent_2 have Time≥1 AND Energy≥4?
   - Public Budget: Is total_pooled_Budget (sum of all agents' observed Budget) ≥ 100+50=150?
     **Remember**: Use the TOTAL pooled public resources (all agents' observations combined), not just your own observation.
   
   If ANY check fails, the proposal violates constraints!
...
TASK_DESCRIPTION_NO_TOOLS_FULL_OBS = "Full-Obs Deliberation Ablation Study (aligned with Baseline Oracle prompt format)"


if __name__ == '__main__':
    print("Task Allocation Level 1 Prompts (No Tool Version) loaded successfully.")
    print(f"OBSERVE_PROMPT length: {len(OBSERVE_PROMPT)}")
    print(f"THINK_PROMPT length: {len(THINK_PROMPT)}")
    print(f"DECISION_PROMPT length: {len(DECISION_PROMPT)}")
    print(f"TALK_PROMPT length: {len(TALK_PROMPT)}")
    print(f"THINK_PROMPT_FULL_OBS length: {len(THINK_PROMPT_FULL_OBS)}")
    print(f"DECISION_PROMPT_FULL_OBS length: {len(DECISION_PROMPT_FULL_OBS)}")
    print(f"TALK_PROMPT_FULL_OBS length: {len(TALK_PROMPT_FULL_OBS)}")
